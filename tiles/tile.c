...
#ifndef	lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/tiles/tile.c,v 1.8 2008/03/06 03:54:51 tim Exp $";
#endif	/* not lint */
...
getTileFromTileStore()
{
	...
	if (TileStoreFreeList->ti_client == CLIENTDEFAULT)
	{
	    TxError("Corrupted tile list at tile 0x%x!\n", TileStoreFreeList);
	    return NULL;
	}
...
tiPrint(tp)
    Tile *tp;
{
    printf("tp=0x%p LL=(%d,%d) body=0x%lu\n",
	tp, LEFT(tp), BOTTOM(tp), tp->ti_body);
    printf("BL=0x%p LB=0x%p RT=0x%p TR=0x%p\n", BL(tp), LB(tp), RT(tp), TR(tp));
}
